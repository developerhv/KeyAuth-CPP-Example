...
namespace KeyAuth {
	...
	private:
		...
		void load_user_data ( nlohmann::json data ) {
			const std::string key_username = encrypt ( "username" );
			const std::string key_ip = encrypt ( "ip" );
			const std::string key_hwid = encrypt ( "hwid" );
			const std::string key_created = encrypt ( "createdate" );
			const std::string key_lastlogin = encrypt ( "lastlogin" );
			const std::string key_subs = encrypt ( "subscriptions" );
			const std::string key_sub_name = encrypt ( "subscription" );
			const std::string key_sub_expiry = encrypt ( "expiry" );
			...
			if ( !data.contains ( key_hwid ) || data [ key_hwid ].is_null ( ) ) {
				api::user_data.hwid = encrypt ( "none" );
			}
			else {
		...
		void load_app_data ( nlohmann::json data ) {
			api::app_data.numUsers = data [ encrypt ( "numUsers" ) ];
			api::app_data.numOnlineUsers = data [ encrypt ( "numOnlineUsers" ) ];
			api::app_data.numKeys = data [ encrypt ( "numKeys" ) ];
			api::app_data.version = data [ encrypt ( "version" ) ];
			api::app_data.customerPanelLink = data [ encrypt ( "customerPanelLink" ) ];
		}

		void load_response_data ( nlohmann::json data ) {
			api::response.success = data [ encrypt ( "success" ) ];
			api::response.message = data [ "message" ];
			api::response.isPaid = false;

			if ( data.contains ( encrypt ( "role" ) ) && data [ encrypt ( "role" ) ] != encrypt ( "tester" ) && data [ encrypt ( "role" ) ] != encrypt ( "not_checked" ) ) {
				api::response.isPaid = true;
			}
		}

		void load_channel_data ( nlohmann::json data ) {
			api::response.success = data [ encrypt ( "success" ) ]; // intentional. Possibly trick a reverse engineer into thinking this string is for login function
			api::response.message = data [ "message" ];
			...
			const std::string key_author = encrypt ( "author" );
			const std::string key_timestamp = encrypt ( "timestamp" );
