#pragma once
#include <cstdint>
#ifndef OXORANY_SINGLE_H
#define OXORANY_SINGLE_H


// Configuration options
// Disable /GL for warning C4307 in msvc
#define OXORANY_DISABLE_OBFUSCATION
// Use OXORANY_USE_BIT_CAST to remove float/double source data
...
#define oxorany_flt
#else
#define encrypt(any) _lxy_oencryptny_::oencryptny<decltype(_lxy_oencryptny_::typeofs(any)), _lxy_oencryptny_::array_size(any), __COUNTER__>(any, _lxy_::make_index_sequence<sizeof(decltype(any))>()).get()
...
namespace _lxy_oencryptny_ {
	...
	*/
	...
	// Static variables implementation (previously in oxorany.cpp)
...
#define random_constant random_constant_32
	...
	static OXORANY_FORCEINLINE constexpr _lxy__uint8_t rol8 ( _lxy__uint8_t v , unsigned r ) {
		return ( _lxy__uint8_t )( ( v << r ) | ( v >> ( 8 - r ) ) );
	}

	static OXORANY_FORCEINLINE constexpr _lxy__uint8_t ror8 ( _lxy__uint8_t v , unsigned r ) {
		return ( _lxy__uint8_t )( ( v >> r ) | ( v << ( 8 - r ) ) );
	}


	template<_lxy__size_t key>
	static OXORANY_FORCEINLINE constexpr _lxy__uint8_t encrypt_byte (
		_lxy__uint8_t c , _lxy__size_t i )
	{
		constexpr _lxy__uint8_t k = ( _lxy__uint8_t )( key * 7 );
		_lxy__uint8_t x = ( c + k ) ^ ( _lxy__uint8_t )( i + key );
		return rol8 ( x , ( key + i ) & 7 );
	}


	template<_lxy__size_t key>
	static OXORANY_FORCEINLINE constexpr _lxy__uint8_t decrypt_byte (
		_lxy__uint8_t c , _lxy__size_t i )
	{
		constexpr _lxy__uint8_t k = ( _lxy__uint8_t )( key * 7 );

		_lxy__uint8_t x = ror8 ( c , ( key + i ) & 7 );

		_lxy__size_t a = x;
		_lxy__size_t b = i + key;
		_lxy__size_t a_xor_b = ( a + b ) - ( ( a & b ) + ( b & a ) );

		return ( _lxy__uint8_t )( a_xor_b - k );
	}
			...
			if ( stack_x == 0 ) {
				for ( volatile _lxy__size_t i = 0; i < size; i++ ) {
					if ( stack_x == stack_y ) {
						...
						if ( stack_x == stack_y ) {
						loc_unreachable_0:
							buffer [ i ] = decrypt_byte<key> ( source , i );
							continue;
						}
						else {
							//unreachable
	...
	public:
		...
		OXORANY_FORCEINLINE constexpr oencryptny ( const any_t ( &any ) [ ary_size ] , _lxy_::index_sequence<indices...> ) :
	...
	class oencryptny<any_t , 0 , counter> {
	...
	public:
		...
		template<_lxy__size_t... indices>
		OXORANY_FORCEINLINE constexpr oencryptny ( any_t any , _lxy_::index_sequence<indices...> ) :
